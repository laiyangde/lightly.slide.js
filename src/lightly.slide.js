var lightlySlide = function(container, params) {
...
lightlySlide.fn.onTouchMove = function(e){
	if (e.targetTouches && e.targetTouches.length > 1) {this.removeTouch();return;}
...
lightlySlide.fn.onTouchEnd = function(e){
	...
	// Find current slide
	var i, stopIndex = 0, groupSize = this.slideSize;
	...
	if (timeDiff > this.params.longSwipesMs) {
	    ...
	    if (this.swipeDirection === 'next') {
	        if (ratio >= this.params.longSwipesRatio) this.slideTo(stopIndex + this.params.slidesPerGroup);
	        else this.slideTo(stopIndex);

	    }
	    if (this.swipeDirection === 'prev') {
	        if (ratio > (1 - this.params.longSwipesRatio)) this.slideTo(stopIndex + this.params.slidesPerGroup);
	        else this.slideTo(stopIndex);
	    }
	}
	else {
